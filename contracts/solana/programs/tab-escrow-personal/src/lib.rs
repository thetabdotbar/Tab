//! TabEscrowPersonal — 0%-fee variant of TabEscrow, dedicated to bot-
//! relayed P2P tips on Solana.
//!
//! Mirrors `tab-subscription-personal` for the escrow surface: same
//! code as the business-tier `tab-escrow` program but with the fee
//! cap hardcoded to 0 so the admin can never accidentally flip it to
//! a non-zero fee. Routing happens by *which program* a tip flows
//! through, not a global toggle — matches the EVM Personal Escrow
//! Router pattern in tab-evm/script/DeployPersonalEscrow.s.sol.
//!
//! Two flows (identical to the business program):
//!
//! 1. **Handle escrow** — sender locks tokens against a Tab handle.
//!    An off-chain executor releases the funds to the recipient's
//!    wallet after off-chain verification.
//!
//! 2. **Code escrow** — sender locks tokens against a secret. Anyone
//!    who reveals the secret to `claim_code` claims the funds. Used
//!    by the bot relay for tips to unlinked recipients.
//!
...
//! changes don't retroactively change an existing escrow's fee.
//! Pause halts new escrow creation only — claims and refunds always
//! work so user funds are never trapped.
...
// Program ID derived from target/deploy/tab_escrow_personal-keypair.json.
// Generated 2026-05-24. Keep this in sync with Anchor.toml.
declare_id!("A583E27BuN3YWP5WmsoFeaYdPtnKMAGU7tHgMrZbqwn");

pub const BPS_DENOMINATOR: u64 = 10_000;
// Personal-tier program: hard cap fee at zero so the admin can never
// flip the personal escrow router to a non-zero fee. Business-tier
// `tab-escrow` program keeps the 500 bps cap.
pub const MAX_FEE_BPS: u64 = 0;
pub const MIN_DEADLINE_WINDOW_SECS: i64 = 3_600; // 1h
pub const MAX_DEADLINE_WINDOW_SECS: i64 = 90 * 24 * 3_600; // 90 days

#[program]
pub mod tab_escrow_personal {
    use super::*;
